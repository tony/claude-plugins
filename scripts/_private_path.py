"""Private path helper that hides the user's home directory in output.

Adapted from the ``PrivatePath`` pattern used in vcspull and tmuxp.
"""
...
if t.TYPE_CHECKING:
    ...
    PrivatePathBase = type(pathlib.Path())  # pyright: ignore[reportUnreachable]
...
class PrivatePath(PrivatePathBase):
    ...
    >>> PrivatePath(home)
    PrivatePath('~')
    >>> PrivatePath(home / "projects" / "my-repo")
    PrivatePath('~/projects/my-repo')
